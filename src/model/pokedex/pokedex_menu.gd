...
func _count_total() -> int:
	match _mode:
		...
		DexMode.REGION:
			...
			return _count_species_in_pokedex(pid)
...
func _index_of_species_in_current_mode(species_id: int) -> int:
	match _mode:
		...
		DexMode.REGION:
			return _index_in_pokedex(_get_region_pokedex_id(_region_id), species_id)
...
# --------- Pokedex by region / national ---------

func _get_national_pokedex_id() -> int:
	# tente de trouver le pokedex "national"
	var rows := PokeDb._query("SELECT id FROM entities WHERE resource='pokedex' AND name='national' LIMIT 1;")
	if rows.size() > 0:
		return int((rows[0] as Dictionary).get("id", 0))
	# fallback: 1 si tu es sûr, sinon 0
	return 1
